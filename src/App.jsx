...
export default function KeebLab() {
  ...
  return (
    ...
    >
      <a href="#main" className="sr-only focus:not-sr-only absolute left-4 top-20 bg-white/90 dark:bg-black/70 text-sm px-3 py-2 rounded-md">Skip to main content</a>
      {/* NAVBAR */}
      <header
        ...
        <div className="container h-16 flex items-center justify-between relative">
          ...
          <nav className="flex items-center gap-4">
            <div className="hidden md:flex items-center gap-6">
              ...
              <a href="#" className="hidden md:inline-flex btn btn-primary">Sign up</a>
            </div>
      ...
      {menuOpen && (
        ...
        >
          <div className="px-6 py-4 flex flex-col gap-3">
            ...
            <a href="#" onClick={() => setMenuOpen(false)} className="btn btn-primary w-full text-center">Sign up</a>
          </div>
        </motion.div>
      )}
  ...
  <span className={`block ${isDark ? "text-slate-100" : "text-[#0F172A]"}`}>
    A better way to
  </span>
    ...
    feel your typing
      ...
      <section id="about" className="pb-32">
        ...
        >
          ...
          >
            <h2 className={`text-3xl md:text-4xl font-bold mb-6 ${isDark ? 'text-white' : 'text-slate-900'}`}>About</h2>
            <p
              className={`max-w-3xl text-lg leading-relaxed ${
                isDark ? "text-gray-300" : "text-slate-700"
              }`}
            >
              KeebLab is our tribute to the mechanical keyboard community. We know what it's like to chase the perfect sound profile, to hand-lube a hundred switches, and to seek out that one elusive keycap set.
            </p>
            <p
            ...
            >
              We felt the existing tools didn't respect the hardware we love. So, we engineered KeebLab to be the ultimate companion to your analog passion. It is a high-fidelity environment where precision meets poetry—a place where your typing isn't just measured, it's celebrated.
            </p>
          </div>
        </motion.div>
      </section>
...
>
  KeebLab
                ...
                <span className={`hidden sm:inline text-sm ${isDark ? 'text-gray-300' : 'text-slate-600'}`}>High-fidelity typing practice for analog enthusiasts.</span>
              </div>

              <p className={`mt-4 text-sm ${isDark ? 'text-gray-300' : 'text-slate-700'}`}>
                Built with care by <a href="https://www.linkedin.com/in/abdullah-parvez-565693246/" className="underline hover:opacity-80">Abdullah Parvez</a> — crafting tools that respect the hardware you love.
              </p>

              <div className="mt-6 flex flex-wrap items-center gap-3">
                <a href="#" className="btn btn-primary px-5 py-3">Get Started</a>
                <a href="https://github.com/abdullah-par/keeblab" className="btn btn-ghost px-4 py-3 inline-flex items-center gap-2"><Github size={16} /> Contribute</a>
              </div>
            </div>

            <div className="flex flex-col items-start md:items-end gap-4">
              <div className="flex items-center gap-3">
                <a href="https://github.com/abdullah-par/keeblab" aria-label="GitHub" className={`social-btn ${isDark ? 'bg-white/3 text-white' : 'bg-white text-[#0F172A]'}`}>
                  <Github size={18} />
                </a>
                <a href="https://www.linkedin.com/in/abdullah-parvez-565693246/" aria-label="LinkedIn" className={`social-btn ${isDark ? 'bg-white/3 text-white' : 'bg-white text-[#0F172A]'}`}>
                  <Linkedin size={18} />
                </a>
              </div>

              <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>
                © {new Date().getFullYear()} KeebLab — Crafted with keyboards in mind.
              </p>
            </div>
          </div>
        </div>
